...
public class GeoRectQueue : MonoBehaviour {
	public int nX = 4;
	public int nY = 5;
	...
	private List<Vector3> GenerateVertices()
	{
		...
		for (int y = 0; y < nY ; y++) //we will have n+1 vertices in a row if is n is the number of quads (y direction)
		{
			...
			{
				verts.Add (new Vector3((float)y*edgelength,(float)-x*edgelength,0)); //set to negative for the sake of visualization (preference)
